...
const UserProfile = () => {
    const {state,dispatch} = useContext(UserContext)
     ...
     useEffect(() => {
        fetch(`${process.env.REACT_APP_API}/users/user/${userid}`,{
            headers:{
    ...
    //follow
    const followUser = () => {
        fetch(`${process.env.REACT_APP_API}/users/follow`,{
            method:"put",
            headers:{
    ...
    //unfollow
    const unfollowUser = () => {
        fetch(`${process.env.REACT_APP_API}/users/unfollow`,{
            method:"put",
            headers:{
                "Content-Type":"application/json",
                "Authorization":"Bearer "+localStorage.getItem('jwt') 
            ...
            <div  style={{display:'flex', justifyContent:'space-around',margin:"18px 0px",borderBottom:'1px solid grey'}}>
                ...
                <div style={{marginRight:"10%"}}>
                    ...
                    <div style={{display:'flex', justifyContent:"space-between", width:'112%'}}>
                        <a>{ userProfile.posts.length}posts</a>
                        <a> {userProfile.user.followers.length} followers</a>
                        <a> {userProfile.user.following.length} followings</a>
                    </div>
